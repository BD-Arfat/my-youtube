...
const SearchResultVideoCard = ({ video }) => {
    return (
        <Link to={`/video/${video.videoId}`}>
            <div className="flex flex-col md:flex-row mb-8 md:mb-3 lg:hover:bh-white/[0.1] rounded-xl md:p-4">
                <div className="relative flex shrink-0 h-48 md:h-28 lg:h-40 xl:h-48 w-full md:w-48 lg:w-64 xl:w-80 rounded-xl bg-slate-800 overflow-hidden">
                    <div className="relative h-48 md:h-full md:rounded-xl overflow-hidden">
                        <img className="h-full w-96 object-cover" src={video?.thumbnails?.[0]?.url} alt="" />
                        {video.lengthSeconds && (
                            <VideoLength time={video?.lengthSeconds} />
                        )}
                    </div>
                </div>
                <div className="flex flex-col ml-4 md:ml-6 md:mt-0 overflow-hidden">
